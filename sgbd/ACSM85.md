...
| Field | Value |
| --- | --- |
| ECU | ACSM / Advanced Crash and Safety Management (Airbag Steuergeraet) fuer E85 und E86 |
| ORIGIN | BMW EI-62 Muhr |
| REVISION | 3.000 |
| AUTHOR | BERATA EngineeringConsulting Chafigoulline, BERATA EngineeringConsulting Schieferstein |
| COMMENT | Airbag Steuergeraet fuer K-Bus Anwendungen fuer E85 und E86 |
| PACKAGE | 1.29 |
| SPRACHE | deutsch |
...
- [STATUS_GURTKONTAKTE](#job-status-gurtkontakte) - Zugriff auf Steuergeraete Ein- und Ausgaenge KWP2000: $300X01 InputOutputControlByLocalIdentifier X = Gruppe Modus  : Default Gurtkontaktwerte in [mA] Der Gurtkontakt ist als 2-Draht-Hall Sensor ausgeführt und wird über das zentrale Airbagsteuergerät versorgt Der Passenger Airbag Off Schalter ist als 2-Draht-Hall Sensor ausgeführt und wird über das zentrale Airbagsteuergerät versorgt
...
- [VERRIEGELUNG_LESEN](#job-verriegelung-lesen) - Auslesen des Pruefstempels KWP2000: $22 ReadDataByCommonIdentifier $1000 TestStamp Modus  : Default  BYTE1=BYTE2=BYTE3 =   0 = 0x00 = verriegelt / Airbags sind scharf BYTE1=BYTE2=BYTE3 = 255 = 0xFF = nicht verriegelt
...
| STAT_KNAF_ZK15_VERBAUT | int | KAFS = Knieairbag Fahrer Zuendkreis 15, 0=nicht verbaut,1= verbaut |
| STAT_KNABF_ZK16_VERBAUT | int | KNABF = Knieairbag Beifahrer Zuendkreis 16, 0=nicht verbaut,1= verbaut |
| STAT_ESF_ZK17_VERBAUT | int | ESF = Endbeschlagsstrammer Fahrer Zuendkreis 17, 0=nicht verbaut,1= verbaut |
| STAT_KOSFS_ZK18_VERBAUT | int | KOSFS = Kopfstuetze Fahrer Zuendkreis 18, 0=nicht verbaut,1= verbaut |
| STAT_KOSBFS_ZK19_VERBAUT | int | KOSBFS = Kopfstuetze Beifahrerseite Zuendkreis 19, 0=nicht verbaut,1= verbaut |
...
| STAT_SBE_BEIFAHRER_VERZOEGERUNG | int | SBE_VERZOEGERUNG = Sitzbelegungserkennung Statusverzoegerung Beifahrer, Umschaltung von belegt --&gt; unbelegt : Entprellzeit betraegt fuer die Airbag Beifahrer Ausloesung 2 Minuten !, 0 = keine Verzoegerung, 1 = Verzoegerung |
...
| STAT_ROLLOVERFUNKTION_SENSORIK | int | ROLLOVER_FUNKTION_SENSORIK = Rollover Funktion Sensorik, 0=nicht verbaut,1= verbaut |
...
| STAT_SBR_FUNKTION_BEIFAHRER | int | SBR_FUNKTION_BEIFAHRER = seat belt reminder Beifahrer Diese Warnung wird bei ungegurtetem Beifahrer während der Fahrt ausgegeben, falls die SBR-Warnung für den Beifahrer aktiv codiert ist. In ungegurtetem Zustand wird nach dem Fahrzeugstart erst bei einer zurückgelegten Wegstrecke und während der Fahrt nach Ablauf einer bestimmten Zeit gewarnt., 0=nicht verbaut,1= verbaut |
| STAT_KOPPLUNG_SBR_POS_POS_E85 | int | Kopplung_SBR+POS = Gurtschloss im 70 Hex Telegramm, 0=nicht inaktiv,1= aktiv |
...
| STAT_KNABF_ZK16_FREIGABE | int | 0 = gesperrt, 1 = freigegeben ZK = ignition circuit KABF = Knieairbag Beifahrer Vor SW 4.00 wurde dieser ZK mit Kopfstuetze Beifahrer geteilt |
...
Zugriff auf Steuergeraete Ein- und Ausgaenge KWP2000: $300X01 InputOutputControlByLocalIdentifier X = Gruppe Modus  : Default Gurtkontaktwerte in [mA] Der Gurtkontakt ist als 2-Draht-Hall Sensor ausgeführt und wird über das zentrale Airbagsteuergerät versorgt Der Passenger Airbag Off Schalter ist als 2-Draht-Hall Sensor ausgeführt und wird über das zentrale Airbagsteuergerät versorgt
...
| STAT_SITZBELEGUNG_BEIFAHRER_FEHLER | int | SBE Beifahrer/OC3 Sitzbelegung SBE Beifahrer/OC3 Fehler-Status 0: kein Fehler SBE: Sitzbelegungserkennung, SBR - Matte : SeatBeltReminder-Matte, OC3: Occupant Classification 3te Generation |
| STAT_SBE_BEIFAHRER_BELEGT_AIRBAG_ON | int | SBE Beifahrersitz belegt (Airbag On) ab 09/05 Airbag_ON aktiv = 1, deaktiviert = 0 Bemerkung: Die Belegungsschwelle AIRBAG_ON wird ab einer Belastung von &gt; 12 Kg erreicht |
| STAT_SBE_BEIFAHRER_BELEGT_AIRBAG_ON_SBR | int | SBE Beifahrersitz belegt (Airbag On + SBR) ab E6x-09-05-350 Airbag_ON + SBR aktiv = 1, deaktiviert = 0 Bemerkung: Die Belegungsschwelle AIRBAG_ON_SBR wird erst ab -Person erkannt-  erreicht |
| STAT_SBE_BEIFAHRER_VERBAUT | int | SBE Beifahrer Sitzbelegung 0: System nicht verbaut 1: System verbaut |
| STAT_OC3_VERBAUT | int | OC3 Sitzbelegung 0: System nicht verbaut 1: System verbaut |
...
| JOB_STATUS | string | OKAY, wenn fehlerfrei table JobResult STATUS_TEXT |
| _TEL_ANTWORT | binary | Hex-Antwort von SG |
| TYP | string | Baureihe z.B: E60 / 20h , E61 / 21h , E63 / 22h , E64 / 23h , E85 / 30h , E86 / 31h.. Vehicle e.g:  E60 / 20h , E61 / 21h , E63 / 22h , E64 / 23h , E85 / 30h , E86 / 31h.. |
...
| ID_PARAMETER | binary | Ausgabe Algo - Versionsnummer |
...
Auslesen des Pruefstempels KWP2000: $22 ReadDataByCommonIdentifier $1000 TestStamp Modus  : Default  BYTE1=BYTE2=BYTE3 =   0 = 0x00 = verriegelt / Airbags sind scharf BYTE1=BYTE2=BYTE3 = 255 = 0xFF = nicht verriegelt
