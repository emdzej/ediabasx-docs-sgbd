...
- [STATUS_STEUERN_RACE_CALIB_DATEN](#job-status-steuern-race-calib-daten) - KWP2000:    $21 ReadDataByLocalIdentifier Request Service Id $3b WriteDataByLocalIdentifier Request Service Id $61, 68-6e recordLocalIdentifier  Über den SGBD-Job STATUS_STEUERN_RACE_CALIB_DATEN können die Kalibrierungsdaten aus dem Steuergerät ausgelesen werden. Hierbei kann über das Argument DATENSATZNUMMER ausgewählt werden, ob der aktuell gültige oder ein anderer Datensatz der Historie ausgelesen werden soll. Es ist also möglich, über diesen Job nicht nur Datensätze auszulesen, die älter als der aktuell gültige Datensatz sind, sondern auch solche, die jünger aber unvollständig oder anderweitig ungültig sind. Generell können alle auch gegebenenfalls auch nur teilweise vorhandene und damit ungültige Datensätze ausgelesen werden, solche Datensätze werden dementsprechend auch nicht übersprungen.  Dieser SGBD Job unterscheidet automatisch zwischen RCT1 und RCT2: Wenn RLI_RACECAL_INFO die Version 2 ausgibt, dann ist RCT1 vorhanden. Wenn hingegen die Version != 2 ist, der Job RLI_RCT2_INFO reagiert und Version 0x0200 ausgibt, dann ist RCT2 vorhanden. Bei Updates der Versionen in den Jobs RLI_RACECAL_INFO oder RLI_RCT2_INFO muss dieser SGBD Job angepasst werden.
...
<a id="job-status-steuern-race-calib-daten"></a>
### STATUS_STEUERN_RACE_CALIB_DATEN

KWP2000:    $21 ReadDataByLocalIdentifier Request Service Id $3b WriteDataByLocalIdentifier Request Service Id $61, 68-6e recordLocalIdentifier  Über den SGBD-Job STATUS_STEUERN_RACE_CALIB_DATEN können die Kalibrierungsdaten aus dem Steuergerät ausgelesen werden. Hierbei kann über das Argument DATENSATZNUMMER ausgewählt werden, ob der aktuell gültige oder ein anderer Datensatz der Historie ausgelesen werden soll. Es ist also möglich, über diesen Job nicht nur Datensätze auszulesen, die älter als der aktuell gültige Datensatz sind, sondern auch solche, die jünger aber unvollständig oder anderweitig ungültig sind. Generell können alle auch gegebenenfalls auch nur teilweise vorhandene und damit ungültige Datensätze ausgelesen werden, solche Datensätze werden dementsprechend auch nicht übersprungen.  Dieser SGBD Job unterscheidet automatisch zwischen RCT1 und RCT2: Wenn RLI_RACECAL_INFO die Version 2 ausgibt, dann ist RCT1 vorhanden. Wenn hingegen die Version != 2 ist, der Job RLI_RCT2_INFO reagiert und Version 0x0200 ausgibt, dann ist RCT2 vorhanden. Bei Updates der Versionen in den Jobs RLI_RACECAL_INFO oder RLI_RCT2_INFO muss dieser SGBD Job angepasst werden.

#### Arguments

| Name | Type | Comment |
| --- | --- | --- |
| DATENSATZNUMMER | int | = 0: aktiver Kalibrierungsdatensatz. &lt; 0: älterer Kalibrierungsdatensatz. = -128: ältester Kalibrierungsdatensatz unabhängig von der tatsächlichen Anzahl &gt; 0: jüngerer Kalibrierungsdatensatz, der noch unvollständig oder als fehlerhaft gesperrt ist. = 127: jüngster Kalibrierungsdatensatz, der momentan oder als nächster beschrieben wird. |

#### Results

| Name | Type | Comment |
| --- | --- | --- |
| STAT_DATA_VALID | unsigned char | 1, falls Datensatz gültig, 0, falls nicht |
| STAT_DATA_VERSION | unsigned int | Version der Kalibrierungsdaten |
| STAT_PROG_CNT | unsigned long | Zähler der Programmierung |
| STAT_KL15_CNT | unsigned long | Klemme 15 - Zähler bei Abspeicherung |
| STAT_KM_CNT_WERT | unsigned long | Kilometerstand bei Abspeicherung |
| STAT_KM_CNT_EINH | string | Kilometerstand bei Abspeicherung |
| STAT_CW_INJ | unsigned int | Bit 0: Lambdaregelung abschalten Bit 1: Schubabschaltung abschalten Bit 2: FGRUCORR einrechnen |
| STAT_FGRUCORR | binary | Kennfeld (12x12) Korrektur Einspritzmenge über nmot_w und wdkba_w als Rohdaten |
| STAT_CW_IGN | unsigned int | Bit 0: Klopfregelung abschalten |
| STAT_ZW_OFFSET | binary | Kennfeld (12x12) Korrektur Zündzeitpunkt über nmot_w und wdkba_w als Rohdaten |
| STAT_CW_DTC | unsigned int | Bit 0: Umschaltung auf TRQMTRX_X Bit 1: Einrechnung TRQINCRPM Bit 2: Umschaltung auf ASC_RAI_WHEELY_ON Bit 3: Umschaltung auf TIRERAD_FW Bit 4: Umschaltung auf TIRERAD_DW Bit 5-9: Umschaltung auf TRACCORR_* Bit 10: Umschaltung auf MJU_MODUS Bit 11: Umschaltung auf TRQREDUCE Bit 12: Umschaltung auf ASC_SLIP_SHIFT (ab RCT2) |
| STAT_TIRERAD_FW_WERT | real | Radius Vorderrad in m |
| STAT_TIRERAD_FW_EINH | string | Radius Vorderrad in m |
| STAT_TIRERAD_DW_WERT | real | Radius Hinterrad in m |
| STAT_TIRERAD_DW_EINH | string | Radius Hinterrad in m |
| STAT_TRQMTRX_X | binary | Reduktionskennlinie (21) über tred_w als Rohdaten |
| STAT_TRQINCRPM | binary | Korrekturkennlinie (8) für TRQINCRPM über nmot_w als Rohdaten |
| STAT_ASC_RAI_WHEELY_ON | binary | Schräglagenwinkelkennlinie (8) Abschaltung DTC über fzgm_modus, Auflösung 0,1° als Rohdaten |
| STAT_MJU_MODUS | binary | Kennlinie (8) modusabhängiger Mjuwert über fzg_asc_modus als Rohdaten |
| STAT_TRACCORR_1 | binary | Reifenkonturkennfeld über tirefory_w und v_vradabs |
| STAT_TRACCORR_2 | binary | Reifenkonturkennfeld über tirefory_w und v_vradabs |
| STAT_TRACCORR_3 | binary | Reifenkonturkennfeld über tirefory_w und v_vradabs |
| STAT_TRACCORR_4 | binary | Reifenkonturkennfeld über tirefory_w und v_vradabs |
| STAT_TRQREDUCE | binary | Kennfeld (8x10) Kammausnutzung gewichtet über kammausnutzung_w und nmot_w |
| STAT_ASC_SLIP_SHIFT | binary | Kennlinie (15) Reduktion aus der Vorsteuerung (ab RCT2) |
| STAT_CW_MISC | unsigned int | Bit 0: SASS_CORR einrechnen Bit 1: NMAX_ESSTART einrechnen ab RCT2: Bit 2: NMAX_LNC einrechnen Bit 3: VLIM_LNC einrechnen Bit 4: TRQLAUNCHCON  einrechnen Bit 5: WDKDN_VAR einrechnen Bit 6: FZGM_AUSGABE_ABS verwenden Bit 7: FZGM_AUSGABE_SAF verwenden |
| STAT_SASS_CORR | real | Momentenaufregelung nach Schaltvorgang als Faktor |
| STAT_NMAX_ESSTART_WERT | real | Drehzahlbegrenzung für Boxengasse in U/min |
| STAT_NMAX_ESSTART_EINH | string | Drehzahlbegrenzung für Boxengasse in U/min |
| STAT_NMAX_LNC_WERT | real | ab RCT2: Motordrehzahlbegrenzung bei aktiver Launch-Control |
| STAT_NMAX_LNC_EINH | string | ab RCT2: Motordrehzahlbegrenzung bei aktiver Launch-Control |
| STAT_VLIM_LNC_WERT | real | ab RCT2: Geschwindigkeitsschwelle Deaktivierung Motordrehzahlbegrenzung durch Launch-Control |
| STAT_VLIM_LNC_EINH | string | ab RCT2: Geschwindigkeitsschwelle Deaktivierung Motordrehzahlbegrenzung durch Launch-Control |
| STAT_TRQLAUNCHCON | binary | ab RCT2: Reduktionskennlinie (rel. Reduktion = f(Motordrehzahl)) für die Launch-Control |
| STAT_WDKDN_VAR | binary | ab RCT2: Korrektur Drosselklappenanschlag über dns_w (Schubluftkennlinie) gang- und fzgmodusabhängig |
| STAT_FZGM_AUSGABE | binary | ab RCT2: Wertetabelle, um die CAN-Ausgabe zur Anzeige des aktiven Fahrzeugmodus zu beeinflussen |
| STAT_FZGM_AUSGABE_SAF | binary | ab RCT2: Wertetabelle, um die CAN-Ausgabe zur Steuerung des aktiven Fahrzeugmodus im SAF (SemiAktivesFahrwerk) zu beeinflussen |
| JOB_STATUS | string | OKAY, wenn fehlerfrei table JobResult STATUS_TEXT |
| _TEL_ANTWORT_HIST | binary | Hex-Antwort von SG |
| _TEL_ANTWORT_INFO | binary | Hex-Antwort von SG |
| _TEL_ANTWORT_INFO2 | binary | Hex-Antwort von SG |
| _TEL_ANTWORT_INJ | binary | Hex-Antwort von SG |
| _TEL_ANTWORT_IGN | binary | Hex-Antwort von SG |
| _TEL_ANTWORT_DTC1 | binary | Hex-Antwort von SG |
| _TEL_ANTWORT_DTC2 | binary | Hex-Antwort von SG |
| _TEL_ANTWORT_DTC3 | binary | Hex-Antwort von SG |
| _TEL_ANTWORT_DTC4 | binary | Hex-Antwort von SG |
| _TEL_ANTWORT_MISC | binary | Hex-Antwort von SG |
...
- [FORTTEXTE](#table-forttexte) (106 × 2)
- [FDETAILSTRUKTUR](#table-fdetailstruktur) (7 × 2)
- [FARTTEXTEERWEITERT](#table-farttexteerweitert) (7 × 3)
- [FUMWELTMATRIX](#table-fumweltmatrix) (106 × 5)
- [FARTTYP](#table-farttyp) (4 × 5)
...
| 0x27EE | CAN-Botschaft Status Federung SemiAktivesFahrwerk SAF |
...
| 0x2980 | Fehler Startfreigabe SWT |
...
| --- | --- | --- | --- | --- |
| 0x27ED | 0x08 | 0x0B | 0x0A | 0x09 |
| 0x27EE | 0x08 | 0x0B | 0x0A | 0x09 |
| 0x2980 | 0xFFFF | 0xFFFF | 0xFFFF | 0x0C |
...
| 0x08 | unplausibles Signal oder Wert |
| 0x09 | Timeoutfehler |
| 0x0A | Checksummenfehler |
| 0x0B | Alivefehler |
| 0x0C | Falschbedatung oder fehlender Freischaltcode |
