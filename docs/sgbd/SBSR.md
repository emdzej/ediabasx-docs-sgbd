...
- [STEUERN_SOLLDREHZAHL_EKP](#job-steuern-solldrehzahl-ekp) - Statusvorgabe Solldrehzahl EKP KWP2000: $31 Steuergeraetespezifische Routine starten $23 Solldrehzahl EKP vorgeben in 1/min $xx Solldrehzahl EKP High Byte $xx Solldrehzahl EKP Low Byte $xx Zeitdauer in Sekunden NUR bei Klemme 15 EIN Modus  : Default
- [STEUERN_BETRIEBSSPANNUNG_EKP](#job-steuern-betriebsspannung-ekp) - Statusvorgabe Betriebsspannung EKP KWP2000: $31 Steuergeraetespezifische Routine starten $24 Betriebsspannung EKP vorgeben $xx Betriebsspannung EKP in 0.1V $xx Zeitdauer in Sekunden Bedingung: Klemme 15 EIN Modus  : Default
- [STEUERN_EKP_ERSTFOERDERUNG](#job-steuern-ekp-erstfoerderung) - Vorgabe Erstfoerdermenge EKP KWP2000: $31 Steuergeraetespezifische Routine starten $24 Erstfoerdermenge EKP starten (Vollast) $A0 Betriebsspannung EKP fest auf 16V $xx Zeitdauer EKP in Sekunden Bedingung: Klemme 15 EIN Modus  : Default
- [STEUERN_EKP_FOERDERMENGE](#job-steuern-ekp-foerdermenge) - Vorgeben Foerdermenge EKP KWP2000: $31 Steuergeraetespezifische Routine starten $25 Vorgeben Foerdermenge EKP $xx Foerdermenge in l/h $xx Zeitdauer EKP in Sekunden Bedingung: Klemme 15 EIN Modus  : Default
- [STEUERN_EKP_PWM_FREQ_MODUS](#job-steuern-ekp-pwm-freq-modus) - EKP PWM Frequenz-Modus setzen KWP2000: $31 Steuergeraetespezifische Routine starten $3D Statusvorgaben zurücknehmen $xx EKP PWM Frequenzmodus $xx Testzeit in Sekunden Bedingung: Klemme 15 EIN EKP: Elektrische Kraftstoffpumpe PWM: Pulse Width Modulation Modus  : Default
...
<a id="job-steuern-solldrehzahl-ekp"></a>
### STEUERN_SOLLDREHZAHL_EKP

Statusvorgabe Solldrehzahl EKP KWP2000: $31 Steuergeraetespezifische Routine starten $23 Solldrehzahl EKP vorgeben in 1/min $xx Solldrehzahl EKP High Byte $xx Solldrehzahl EKP Low Byte $xx Zeitdauer in Sekunden NUR bei Klemme 15 EIN Modus  : Default

#### Arguments

| Name | Type | Comment |
| --- | --- | --- |
| SOLLDREHZAHL | long | Solldrehzahl EKP in 1/min Bereiche 0 bis FFFF min. und max. sind abhänging vom Pumpentyp min. 800, max. 6000, gesehen über alle Pumpentypen! |
| ZEITDAUER_EKP_DREH | unsigned int | Zeitdauer in Sekunden Bereiche 0 bis 255 |
...
| --- | --- | --- |
| JOB_STATUS | string | OKAY, wenn fehlerfrei table JobResult STATUS_TEXT |
| STAT_AKTUELLE_DREHZAHL_WERT | unsigned int | Aktuelle Drehzahl in 1/min |
| STAT_AKTUELLE_DREHZAHL_EINH | string | Aktuelle Drehzahl in 1/min |
| _TEL_ANTWORT | binary | Hex-Antwort von SG |

<a id="job-steuern-betriebsspannung-ekp"></a>
### STEUERN_BETRIEBSSPANNUNG_EKP

Statusvorgabe Betriebsspannung EKP KWP2000: $31 Steuergeraetespezifische Routine starten $24 Betriebsspannung EKP vorgeben $xx Betriebsspannung EKP in 0.1V $xx Zeitdauer in Sekunden Bedingung: Klemme 15 EIN Modus  : Default

#### Arguments

| Name | Type | Comment |
| --- | --- | --- |
| BETRIEBSSPANNUNG | unsigned int | BETRIEBSSPANNUNG EKP in 0.1V Bereiche 0 bis 160 |
| ZEITDAUER_EKP_SPG | unsigned int | Zeitdauer in Sekunden Bereiche 0 bis 255 |
...
| --- | --- | --- |
| JOB_STATUS | string | OKAY, wenn fehlerfrei table JobResult STATUS_TEXT |
| STAT_AKTUELLE_EKP_SPG_WERT | real | Aktuelle Spannung in V |
| STAT_AKTUELLE_EKP_SPG_EINH | string | Aktuelle Spannung in V |
| _TEL_ANTWORT | binary | Hex-Antwort von SG |

<a id="job-steuern-ekp-erstfoerderung"></a>
### STEUERN_EKP_ERSTFOERDERUNG

Vorgabe Erstfoerdermenge EKP KWP2000: $31 Steuergeraetespezifische Routine starten $24 Erstfoerdermenge EKP starten (Vollast) $A0 Betriebsspannung EKP fest auf 16V $xx Zeitdauer EKP in Sekunden Bedingung: Klemme 15 EIN Modus  : Default

#### Arguments

| Name | Type | Comment |
| --- | --- | --- |
| ZEITDAUER_EKP_ERST | unsigned int | Zeitdauer in Sekunden Bereiche 0 bis 255 |
...
<a id="job-steuern-ekp-foerdermenge"></a>
### STEUERN_EKP_FOERDERMENGE

Vorgeben Foerdermenge EKP KWP2000: $31 Steuergeraetespezifische Routine starten $25 Vorgeben Foerdermenge EKP $xx Foerdermenge in l/h $xx Zeitdauer EKP in Sekunden Bedingung: Klemme 15 EIN Modus  : Default

#### Arguments

| Name | Type | Comment |
| --- | --- | --- |
| FOERDERMENGE_EKP | unsigned int | Foerdermenge in l/h Bereiche 50 bis 150 |
| ZEITDAUER_EKP_FOERDER | unsigned int | Zeitdauer in Sekunden Bereiche 0 bis 255 |
...
| --- | --- | --- |
| JOB_STATUS | string | OKAY, wenn fehlerfrei table JobResult STATUS_TEXT |
| STAT_AKTUELLE_DREHZAHL_FOERDER_WERT | unsigned int | Aktuelle Drehzahl in 1/min |
| STAT_AKTUELLE_DREHZAHL_FOERDER_EINH | string | Aktuelle Drehzahl in 1/min |
| _TEL_ANTWORT | binary | Hex-Antwort von SG |

<a id="job-steuern-ekp-pwm-freq-modus"></a>
### STEUERN_EKP_PWM_FREQ_MODUS

EKP PWM Frequenz-Modus setzen KWP2000: $31 Steuergeraetespezifische Routine starten $3D Statusvorgaben zurücknehmen $xx EKP PWM Frequenzmodus $xx Testzeit in Sekunden Bedingung: Klemme 15 EIN EKP: Elektrische Kraftstoffpumpe PWM: Pulse Width Modulation Modus  : Default

#### Arguments

| Name | Type | Comment |
| --- | --- | --- |
| EKP_PWM_FREQUENZMODUS | unsigned int | EKP PWM Frequenzmodus EKP: Elektrische Kraftstoffpumpe PWM: Pulse Width Modulation 0/3: Normalmodus 1: Tieffrequenzmodus 2: Hochffrequenzmodus |
| MODUS_TESTZEIT | unsigned int | Testzeit in Sekunden |
...
| STAT_EKP_IST_DREHZAHL_WERT | unsigned int | Elektrische Kraftstoffpumpe, Ist-Drehzahl |
| STAT_EKP_IST_DREHZAHL_EINH | string | Elektrische Kraftstoffpumpe, Ist-Drehzahl Einheit: 1/min |
| STAT_EKP_SOLL_DREHZAHL_WERT | unsigned int | Elektrische Kraftstoffpumpe, Soll-Drehzahl |
| STAT_EKP_SOLL_DREHZAHL_EINH | string | Elektrische Kraftstoffpumpe, Soll-Drehzahl Einheit: 1/min |
| STAT_EKP_SPANNUNG_WERT | real | Elektrische Kraftstoffpumpe, Spannung |
| STAT_EKP_SPANNUNG_EINH | string | Elektrische Kraftstoffpumpe, Spannung Einheit: V |
| STAT_EKP_STROM_WERT | real | Elektrische Kraftstoffpumpe, Strom 8 Zylinder Motor: min.: 3A, max.: 10A, mittel: 8A 12 Zylinder Motor: min.: 3A, max.: 12A |
| STAT_EKP_STROM_EINH | string | Elektrische Kraftstoffpumpe, Strom Einheit: A |
| STAT_EKP_TEMPERATURSENSOR_WERT | unsigned int | EKP-Spannungsversorgung, Temperatursensor EKP: Elektrische Kraftstoffpumpe Nur Byteweise, weil nicht linear |
| STAT_EKP_FREQUENZ_MODUS | unsigned int | (Bit 0:) 0 =  Tief Frequenzmodus 1 =  Hoch Frequenzmodus |
| STAT_EKP_PWM_MODUS_WERT | unsigned int | EKP: Elektrische Kraftstoffpumpe PWM: Pulse Width Modulation (Bits 4,5:) 0/3 =  Normaler Betriebsmodus 1 =  Fest im Tieffrequenzmodus 2 =  Fest im Hochfrequenzmodus |
| STAT_EKP_PWM_MODUS_TEXT | string | Aktueller EKP PWM Ansteuerung aus Codierdatei EKP: Elektrische Kraftstoffpumpe PWM: Pulse Width Modulation (Bits 4,5:) 0/3 =  Normaler Betriebsmodus 1 =  Fest im Tieffrequenzmodus 2 =  Fest im Hochfrequenzmodus |
| _TEL_ANTWORT | binary | Hex-Antwort von SG |
...
| Name | Type | Comment |
| --- | --- | --- |
| STAT_ZK1_AKTIV | int | 0=ZK1_NICHT_AKTIV 1=ZK1_AKTIV Gurtkraftbegrenzer vorne rechts |
| STAT_ZK2_AKTIV | int | 0=ZK2_NICHT_AKTIV 1=ZK2_AKTIV Sicherheitsbatterieklemme |
...
| 0x98b1 | Timeout ID 05H (SBSL) |
| 0x98b2 | Timeout ID 06H (SASL) |
| 0x98b3 | Timeout ID 07H (SBSR) |
| 0x98b4 | Timeout ID 08H (SASR) |
| 0x98b5 | Timeout ID 09H (SFZ) |
| 0x98b6 | Timeout ID 0AH (SIM) |
| 0x98b7 | Timeout ID 0BH (SASL) |
| 0x98b8 | Timeout ID 0CH (SASR) |
| 0x98b9 | Timeout ID 0DH (SFZ) |
| 0x98ba | Timeout ID 0EH |
| 0x98bb | Timeout ID 0FH |
| 0x98bc | Timeout ID 20H (SSFA) |
| 0x98bd | Timeout ID 21H (SSBF) |
| 0x98be | Timeout ID 22H (SSH) |
| 0x98bf | Timeout ID 71H (SIM) |
| 0x98c0 | Codierdaten Checksummenfehler |
| 0x98c1 | Stack Overflow |
...
| 0x98ce | Zuendkapazitaet ZK1 n.i.O. |
...
| 0x98d9 | Zuendkapazitaet ZK2 n.i.O. |
| 0x98da | Codierung/Konfiguration ZK2 unstimmig |
| 0x98db | Unterbrechung Entladekreis ZK2 |
| 0x98dc | Fehler Beschleunigungssensor:Offset zu gross |
| 0x98dd | Fehler Beschleunigungssensor:Selbsttestwert zu gross |
| 0x98de | Fehler Beschleunigungssensor:Selbsttestwert zu klein |
| 0x98df | Statusfehler Beschleunigungssensor |
| 0x98e0 | Klemme 30 fehlt |
| 0x98e2 | Fehler im Alarmpfad |
| 0x98e3 | Unterbrechung EKP |
| 0x98e4 | EKP leichtgaengig |
| 0x98e5 | EKP schwergaengig |
| 0x98e6 | EKP abgeschaltet (Crash) |
| 0x98e7 | Asic defekt |
| 0x98e8 | Asic in einem ungueltigen Zustand |
| 0x98ea | Asic Uebertemperaturabschaltung Low-Side-Transistor |
| 0x98eb | Asic Uebertemperaturabschaltung High-Side-Transistor |
| 0x98ec | Algorithmus-Parameter inkonsistent |
| 0x98ed | EAM-Parameter inkonsistent |
| 0x98ee | Zuendversuch erfolgt |
| 0x98ef | COP-Watchdog fehlerhaft |
| 0x98f9 | Kurzschluss oder Unterbrechung Low-Side-Schalter ZK1 |
| 0x98fa | Kurzschluss oder Unterbrechung High-Side-Schalter ZK1 |
| 0x98fb | Kurzschluss oder Unterbrechung Low-Side-Schalter ZK2 |
| 0x98fc | Kurzschluss oder Unterbrechung High-Side-Schalter ZK2 |
| 0x98fd | Kommunikation mit Zuend-IC gestoert |
| 0x98fe | Spannungsueberwachung ASE unterer Grenzwert unterschritten |
| 0x98ff | Spannungsueberwachung ASE oberer Grenzwert ueberschritten |
| 0x9a00 | Energiesparmode aktiv |
| 0x9a03 | Codierung unvollstaendig |
| 0x98e1 | Klemme 15 unplausibel |
| 0x98e9 | EKP Notlaufbetrieb (keine Motordaten) |
...
| 0x98e1 | Klemme 15 unplausibel |
| 0x98e9 | EKP Notlaufbetrieb (keine Motordaten) |
| 0x98f0 | Power-On-Reset uP |
| 0x98f1 | Diagnose S/E-Modul (Lichtleistung) |
| 0x98f2 | SI-Bus: Uebertragungsfehler, CRC-Fehler ERRIF |
| 0x98f3 | SI-Bus: Uebertragungsfehler, Formatfehler ILLPIF |
| 0x98f4 | SI-Bus: Synchronisierungspuls zu frueh SYNEIF |
| 0x98f5 | SI-Bus: Synchronisierung verloren SYNLIF |
| 0x98f6 | SI-Bus: Slotmismatch, Timingfehler SLMMIF |
| 0x98f7 | SI-Bus: FIFO-Ueberlauf OVRNIF |
| 0x98f8 | EMV-Fehler im Zuend-Ic |
| 0x9a01 | Klemme 15 unplausibel (Bus-Klemme ist AUS) |
| 0x9a02 | Klemme 15 unplausibel (Bus-Klemme ist EIN) |
| 0x9a04 | EKP Notlaufbetrieb (ungueltige Motordaten) |
| 0x???? | unbekannter Fehlerort |
